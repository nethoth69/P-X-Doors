...
async function notifyNewOrder(order) {
  ...
  try {
    ...
    await transport.sendMail({
      ...
      subject: `New Pixelxcript Installations order — ${order.orderNumber}`,
...
function buildWhatsAppLink(order) {
  ...
  const text = encodeURIComponent(`New Pixelxcript Installations order ${order.orderNumber} from ${order.customer.name} (${order.customer.phone}) — total GHS ${order.total.toLocaleString('en-GH', { minimumFractionDigits: 2 })}`);
